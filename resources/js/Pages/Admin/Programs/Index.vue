                ...
                <tbody>
                  <tr v-for="(program, index) in programs.data" :key="program.id" 
                    ...
                    <td class="px-6 py-4 whitespace-no-wrap text-sm leading-5 font-medium text-gray-900">
                      <div v-show="programOffset !== program.id">
                        {{ program.name }}
                        <button class="px-3 py-2 text-teal-700 hover:text-teal-500" @click="startEditing(program.id)"><i class="fad fa-edit"></i></button> 
                      </div>
                      <div v-show="programOffset === program.id" class="flex items-start justify-start">
                        <form class="flex items-start" @submit.prevent="valueUpdate(program)">
                          <text-input v-model="program.name" :errors="error('name', 'program.' + program.id)" class="pr-2" />
                    ...
                    <td class="px-6 py-4 whitespace-no-wrap text-center text-sm leading-5 text-gray-500">
                      {{ program.datasets.length }}
                    </td>
                    <td class="px-6 py-4 whitespace-no-wrap text-right text-sm leading-5 text-gray-500">
                      <button class="px-3 py-2 text-red-700 hover:text-red-500" v-show="program.datasets.length === 0" @click="destroyregion(program.id)"><i class="fad fa-trash-alt"></i></button>
                    </td>
                  </tr>
                </tbody>
                <tfoot>
...
export default {
  ...
  methods: {
    ...
    valueUpdate(region) {
      this.$inertia.put(this.route('programs.update', region), region)
        .then(() => {
          if (!this.$page.errors['program.'+region.id]) {
            this.programOffset = -1
          }
        })
    },
    destroyregion(id) {
      if (confirm('Are you sure you want to delete this program')) {
        this.$inertia.delete(this.route('programs.destroy', id))
      }
    },
    submit() {
      this.sending = true
      this.$inertia.post(this.route('programs.store'), this.programForm)
        .then(() => {
